...
const BattlePage = () => {
    ...
    useEffect(() => {
        ...
        .then(res => {
            ...
            const dungeonDeck = cardsRes.data.filter((card: any) => cardIds.includes(card.id))
            setDungeonCards(dungeonDeck)
            })
        })
        .catch(err => console.error("Hiba a dungeon kártyák lekérésekor:", err))
    ...
    const getAffinityName = (aff: number) => {
        switch (aff) {
            ...
            case 2: return "Föld"
            case 3: return "Víz"
            case 4: return "Levegő"
            default: return "Ismeretlen"
        }
    }


    const renderCard = (card: GameCard, color: string) => (
        <Card key={card.id} className={`CreatedCard p-2 w-full max-h-fit min-w-[245px] border-${color}-400 bg-${color}-50`}>
    ...
    <div className="min-h-screen bg-background p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-10">
          <div className="flex-1">
            <h2 className="text-lg font-semibold mb-2 text-center">Játékos paklija</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {playerDeck?.map((card: GameCard) => renderCard(card, "blue"))}
            </div>
          </div>

          <div className="flex-1">
            <h2 className="text-lg font-semibold mb-2 text-center">Kazamata kártyái</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {dungeonCards.map((card: GameCard) => renderCard(card, "red"))}
            </div>
          </div>
        </div>

        <div className="flex justify-center mt-10">
          <Button onClick={startBattle}>Harc megkezdése</Button>
        </div>
